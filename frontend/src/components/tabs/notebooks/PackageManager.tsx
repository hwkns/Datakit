    ...
    return (
      <div className="h-full flex items-center justify-center p-4">
        <div className="text-center">
          ...
          <p className="text-white/70">{t('notebooks.packageManager.pythonNotInitialized', { defaultValue: 'Python not initialized' })}</p>
          <p className="text-sm text-white/50">{t('notebooks.packageManager.initializePython', { defaultValue: 'Initialize Python to manage packages' })}</p>
        </div>
      </div>
    );
  }
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="p-4 border-b border-white/10">
        <div className="flex items-center gap-2 mb-3">
          ...
          <h3 className="font-medium text-white">{t('notebooks.packageManager.title', { defaultValue: 'Package Manager' })}</h3>
        </div>
        ...
        <div className="relative mb-3">
          ...
          <input
            type="text"
            placeholder={t('notebooks.packageManager.searchPlaceholder', { defaultValue: 'Search packages...' })}
        ...
        <div className="flex items-center gap-2">
          ...
          >
            {t('notebooks.packageManager.showInstalledOnly', { defaultValue: 'Show Installed Only' })}
          </button>
          <span className="text-xs text-white/50">
            {t('notebooks.packageManager.installedCount', { defaultValue: '{{count}} installed', count: installedPackages.size })}
          </span>
        </div>
      </div>
      ...
      <div className="p-4 border-b border-white/10">
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="text-sm font-medium text-white">{t('notebooks.packageManager.installCustom', { defaultValue: 'Install Custom Package' })}</span>
        </div>
        
        <div className="flex gap-2">
          <input
            type="text"
            placeholder={t('notebooks.packageManager.packageNamePlaceholder', { defaultValue: 'Package name (e.g., requests)' })}
        ...
        <div className="mt-2 text-xs text-white/60 flex items-start gap-1">
          ...
          <span>
            {t('notebooks.packageManager.packageInfo', { defaultValue: 'Only pure Python packages or packages with WebAssembly builds' })}
          </span>
        </div>
      </div>
        ...
        {filteredPackages.length === 0 ? (
          <div className="p-4 text-center">
            ...
            <p className="text-sm text-white/60">
              {searchQuery ? t('notebooks.packageManager.noPackagesFound', { defaultValue: 'No packages found' }) : t('notebooks.packageManager.noPackagesToShow', { defaultValue: 'No packages to show' })}
            </p>
          </div>
        ) : (
                ...
                <div className="flex items-center gap-2">
                  {pkg.isInstalled ? (
                    <div className="flex items-center gap-1 px-2 py-1 bg-green-500/20 text-green-400 rounded text-xs">
                      ...
                      <span>{t('notebooks.packageManager.installed', { defaultValue: 'Installed' })}</span>
                    </div>
                  ) : (
                    <Button
                      variant="ghost"
                    ...
                    >
                      {pkg.isInstalling ? (
                        ...
                        <>
                          ...
                          <span>{t('notebooks.packageManager.install', { defaultValue: 'Install' })}</span>
                        </>
                      )}
                    </Button>
                  )}
        ...
        <div className="border-t border-white/10 p-4">
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm font-medium text-white">
              {t('notebooks.packageManager.installedPackagesTitle', { defaultValue: 'Installed Packages ({{count}})', count: installedPackages.size })}
            </span>
            <Button
              variant="ghost"
              size="sm"
            ...
            >
              {t('notebooks.packageManager.refresh', { defaultValue: 'Refresh' })}
            </Button>
          </div>
          ...
          <div className="max-h-32 overflow-y-auto">
            ...
            {installedPackages.size > 10 && (
              <div className="text-xs text-white/50 text-center mt-2">
                {t('notebooks.packageManager.andMore', { defaultValue: '... and {{count}} more', count: installedPackages.size - 10 })}
              </div>
            )}
          </div>
        </div>
