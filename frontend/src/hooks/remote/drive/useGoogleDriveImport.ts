...
export default function useGoogleDriveImport() {
  ...
  useEffect(() => {
    const initializeGoogleAPI = async () => {
      try {
        ...
        setError(
          "Failed to initialize Google Drive API. Please check your configuration."
        );
      }
    };
  ...
  const fetchFiles = useCallback(async () => {
    ...
    try {
      ...
      console.log(
        `[GoogleDrive] Fetched ${fetchedFiles.length} supported files`
      );
    } catch (err) {
  ...
  const downloadFile = useCallback(
    async (fileId: string, mimeType: string): Promise<Blob> => {
   ...
   */
  const exportGoogleSheet = useCallback(
    async (fileId: string): Promise<Blob> => {
      ...
      if (!response.ok) {
        throw new Error(
          `Failed to export Google Sheet: ${response.statusText}`
        );
      }
   ...
   */
  const downloadAndImportFile = useCallback(
    async (
      fileId: string,
      fileName: string
      ...
      try {
        ...
        console.log(
          `[GoogleDrive] Successfully imported file: ${actualFileName}`
        );
        return result;
      } catch (err) {
